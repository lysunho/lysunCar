...
function createFilter(origin){
	...
	placeholder.innerHTML="请选择"+origin.title;
	origin.option.forEach(function(item){
		var li=document.createElement("li");
		li.innerHTML=item.name;
		option.appendChild(li);
		li.onclick=function(){//li点击的时候做出以下动作：
			placeholder.innerHTML=item.name;
			renderCars(carData.filter(function(_item){
				return origin.filterFunc(_item,item.value);
			}));				
			//2.按照对应的条件筛选出满足条件的汽车，
			//并放在页面上，把原来的页面覆盖掉
		};				
	...
	unlimited.innerHTML="不限";
	unlimited.onclick=function(){
		renderCars(carData);
		placeholder.innerHTML="请选择"+origin.title;
	};
	option.appendChild(unlimited);
...
var filterArray=[
	...
	function(item,value){
	var num=value.split("-");
	return +item.price>=num[0] && +item.price<num[1];	
	}
];
filterData.forEach(function(item,index){
	item.filterFunc=filterArray[index];
	filterFragment.appendChild(createFilter(item));
});
...
var renderCars=function(){
	...
	return function(data){
		...
		data.forEach(function(item){
			carFragment.appendChild(createCarBox(item));
		});
